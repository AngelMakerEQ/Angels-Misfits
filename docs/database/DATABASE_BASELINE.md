...
- Item/spell/NPC-level expansion scoping not yet implemented — deferred
  per ADR-001, to be handled as Phase 3/5 work proceeds.
- TAKP Rebalanced data not yet compared against PEQ baseline values.
- No live-environment verification yet (MCP/HeidiSQL) confirming this
  dump matches the actual Angels Misfits working database state.